...
public partial class MeshCoreMainWindow : Window
{
    ...
    private ConnectionType GetTransport()
    {
        ...
        return tag switch { "Bluetooth" => ConnectionType.Bluetooth, "Tcp" => ConnectionType.Tcp, _ => ConnectionType.Serial };
    }
    ...
    private async void ConnectButton_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            var transport = GetTransport();
            _connection = transport switch { ConnectionType.Bluetooth => CreateBluetoothConnection(), ConnectionType.Tcp => new MeshCoreTcpConnectionService(), _ => new SerialConnectionService() };
    ...
    {
        _startup = null; _client?.Dispose(); _client = null; _connection?.Disconnect(); _connection?.Dispose(); _connection = null;
        _contacts.Clear(); _channels.Clear(); _messages.Clear(); ConnectButton.Content = "Connect"; StatusText.Text = "Disconnected"; DeviceText.Text = "No MeshCore device";
    }
    ...
    private void ClientContactsChanged(object? sender, IReadOnlyList<MeshCoreContact> contacts) => Dispatcher.Invoke(() => { _contacts.Clear(); foreach (var contact in contacts) _contacts.Add(contact); });
    private void ClientChannelReceived(object? sender, MeshCoreChannel channel) => Dispatcher.Invoke(() => { var existing = _channels.FirstOrDefault(x => x.Index == channel.Index); if (existing != null) _channels.Remove(existing); _channels.Add(channel); if (ChannelsListBox.SelectedItem is null && channel.Index == 0) ChannelsListBox.SelectedItem = channel; });
    private void ClientMessageReceived(object? sender, MeshCoreMessage message) => Dispatcher.Invoke(() => _messages.Add($"[{DateTimeOffset.FromUnixTimeSeconds(message.Timestamp):HH:mm:ss}] {message.Text}"));
